...
SUSPICIOUS_MODEL_FILENAME = "models/NDR_suspicious_classifier.sav"
...
def build_default_multiclass_model(random_state=42):
...
def cross_validate_multiclass(X, y, model=None, cv=5):
    if model is None:
        model = build_default_multiclass_model()
...
def train_multiclass_classifier(benign_nrows=10000, malicious_nrows=1000,
...
if __name__ == "__main__":
    ...
    print("Training multiclass classifier and saving to", SUSPICIOUS_MODEL_FILENAME)
    model, label_decoder = train_multiclass_classifier()
