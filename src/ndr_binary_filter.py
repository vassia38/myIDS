...
BINARY_MODEL_FILENAME = "models/NDR_binary_filter.sav"
...
def load_binary_dataset(benign_nrows=10000, malicious_nrows=2500,
    ...
    X = df.drop(columns=["Label"])  # all numeric features selected by utils.load_data
...
def build_default_binary_model(random_state=42):
...
def cross_validate_binary(X, y, model=None, cv=5):
    if model is None:
        model = build_default_binary_model()
...
def train_binary_filter(benign_nrows=20000, malicious_nrows=2500,
...
if __name__ == "__main__":
    ...
    print("Training binary filter model and saving to", BINARY_MODEL_FILENAME)
    model = train_binary_filter()
