...
def analyze():
    try:
        rfc = utils.load_model("models/RF_SMOTE_multi.sav")
        dump_path = Path('out.csv')
        if not dump_path.is_absolute():
            dump_path = Path(__file__).resolve().parent.parent / dump_path
        df = prepare_dump(pd.read_csv(dump_path, header=None))
        X = df.values
        ...
        df.to_csv(output_dir / dump_file)
