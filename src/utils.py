...
def load_data(benigndata_file: str, maliciousdata_folder: str,
    ...
    benign_path = Path(benigndata_file)
    if not benign_path.is_absolute():
        benign_path = REPO_ROOT / benign_path

    malicious_folder = Path(maliciousdata_folder)
    if not malicious_folder.is_absolute():
        malicious_folder = REPO_ROOT / malicious_folder

    df_b = pd.read_csv(benign_path, nrows=benign_nrows, skiprows=range(1, benign_skip_n))
    df = pd.concat([df_b], ignore_index=True)

    for malicious_path in sorted(malicious_folder.iterdir()):
        if malicious_path.suffix.lower() == '.csv':
            basename = malicious_path.name
            if basename.lower().find("worms") != -1:
                if malicious_skip_n == 0:
                    df_m = pd.read_csv(malicious_path, nrows=100, skiprows=range(1, 0))
                else:
                    df_m = pd.read_csv(malicious_path, nrows=64, skiprows=range(1, 100))
            else:
                df_m = pd.read_csv(malicious_path, nrows=malicious_nrows, skiprows=range(1, malicious_skip_n))
...
def load_model(filename="models/RF_classifier.sav") -> RandomForestClassifier:
    return joblib.load(_resolve_model_path(filename))


def save_model(model: RandomForestClassifier, filename="models/RF_classifier.sav"):
    joblib.dump(model, _resolve_model_path(filename))
